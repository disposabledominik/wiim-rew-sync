...
| 88 | Initial device discovery times out (30s) in packaged app, then shows all 4 devices | FIXED | YES | `97eca0c` | Sequential mDNS→subnet scan + redundant enrichment took ~30s on cold start. Rewrote DiscoveryModule: parallel mDNS+subnet scan (1.5s grace), skip redundant getStatusEx for subnet results, progressive UI updates via on_found callback |
| 89 | Cancel button invisible in dark theme (UnsavedChangesDialog) | FIXED | YES | `47087a2` | Removed hardcoded inline styles from Cancel button; uses ghost class from QSS theme |
| 90 | Settings screen paths not pre-populated; theme change + log buttons broken | FIXED | YES | `47087a2` | _apply_settings now passes computed paths (get_log_dir, profile_repository.storage_root) when settings have empty defaults; theme wiring was already correct |
| 91 | Onboarding overlay uses hardcoded COLORS_LIGHT; text clipped in dark mode | FIXED | YES | `47087a2`, `212f672` | Overlay now rebuilds on showEvent to detect active theme; uses COLORS_DARK/COLORS_LIGHT dynamically; Skip removed |
| 92 | Pull from REW: L/R filters pushed to RoomFit profile result in empty/flat bands | FIXED | YES | `7478e7c` | Push path used naive split_lr_filters (50/50) which scrambles unequal-length channels; now stores and uses explicit filters_l/filters_r in WizardState |
| 93 | Pull from REW: subsequent pull after preset load uses stale filters from preset | FIXED | YES | `7478e7c`,`60a07d8` | Root cause: When using the REW API some filter values can have long decimals (can be up to 16 digits after the decimal point - for any of freq, gain or Q). The app passes those directly to WiiM devices, and they don't seem to like that in L/R mode. In Stereo mode the same thing doesn't seem to cause issues. The fix was to round all filter values to 3 decimal places before pushing to WiiM devices. Issue #92 was actually caused by the same root cause. |
| 94 | Connect step loses checkmark/context after RoomFit push via REW API | FIXED | YES | `f5a597c` | _mark_prior_steps_completed now explicitly marks CONNECT as completed when device is selected |
| 95 | Pull from REW: Quick Setup dialog shown unnecessarily when already at Filters step | FIXED | YES | `1e4111d` | _ensure_wizard_state_for_load now checks if user is at/past FILTERS step and skips dialog; PEQ_ONLY flow no longer asks for EQ_TYPE |
| 96 | Pull from REW: measurement picker not shown (REW returns dict-keyed response) | FIXED | YES | `c496aba` | rew_http_client now handles dict-keyed measurement responses (keys "1", "2", ...) |
| 97 | Pull from REW: "Unknown filter type 'None'" error from REW API filters | FIXED | YES | `cbc8ae7` | Unified _TYPE_MAP handles all REW filter types; None/Modal/AllPass/L-T skipped; gain field reads "gaindB" |
| 98 | Push page shows stale DRY RUN content from previous dry run after real push | FIXED | YES | `a2dcafd` | PushPage.reset() called on every PUSH step entry via _on_step_changed |
| 99 | Duplicated WiiM device identification logic in two modules | FIXED | YES | `068caf5` | capability_prober and subnet_scanner had independent device lists with different entries and algorithms; unified into src/utils/device_identity.py |
| 100 | Hardware limit constants (gain/Q) duplicated in translator and GUI | FIXED | YES | `068caf5` | Identical _GAIN_MIN/MAX, _Q_MIN/MAX defined independently in wiim_generator.py and shared_helpers.py; consolidated into src/models/constants.py (indirectly tested via clamping tests) |
...
| 102 | Two crash handlers overwriting each other (logging vs GUI) | FIXED | YES | `068caf5` | install_crash_handler (logging/setup.py) and _crash_handler (main_window.py) both set sys.excepthook; GUI handler now flushes all log handlers for persistence (install_crash_handler never called in production; GUI handler not unit-testable without Qt) |
| 103 | Dead PEQBand model defined but never used in production code | FIXED | YES | `068caf5` | PEQBand in models/peq.py had validators and was exported in __all__ but never instantiated anywhere; removed entirely |
| 104 | Lazy `import json` (3×) inside methods in capability_prober.py | FIXED | YES | `068caf5` | stdlib module imported at function scope for no reason; moved to module-level import |
| 105 | App crashes on startup: `ProfileRepository` has no `storage_root` attribute | FIXED | YES | `ae787c2` | Added `storage_root` property to expose `_profiles_dir.parent` |
...
| 108 | Status banner causes layout jumps (appears/disappears) | FIXED | YES | `e3636e1` | Banner now always reserves space (fixedHeight); uses idle state with transparent styling when empty |
...
| 111 | Sidebar device label click does nothing (tooltip says "Go to Connect step") | FIXED | YES | `212f672` | Was emitting "home" (= current wizard step); changed to emit "connect" with direct navigation to Connect page |
| 112 | Help/User Guide has no visible close button; Diagnostics dock awkward UX | FIXED | YES | `b0c9900`, `2452246` | Both views now open as separate QDialog windows with native OS title bar controls; HelpView removed from stacked widget so content renders |
| 113 | Presets on Device loads stereo preset as L/R when previous load was L/R | FIXED | YES | `212f672` | _do_load_peq_preset never reset channel_mode; now reads channel_mode from PEQ device response before emitting |
| 114 | Status banner close/dismiss button not fully visible (clipped) | FIXED | YES | `212f672`, `2452246` | Banner height 44px; cancel button has explicit border/padding; close button styled for visibility |
...
| 117 | Onboarding overlay always shows light theme; Skip button redundant | FIXED | YES | `212f672`, `2452246` | Removed Skip button; overlay rebuilds on showEvent for theme; increased label min heights |
...
| 120 | REW API L/R: unequal band counts cause right-channel bleed into left | FIXED | YES | `2bd835d` | `bands_l and bands_r` was falsy for empty list; changed to `is not None` check; naive split_lr_filters fallback only used when peq_data lacks explicit L/R |
...
| 124 | Device label click skips wizard reset; navigates to wrong step after re-connect | FIXED | YES | `5c73b2a` | Was navigating directly to stacked widget index; now uses wizard_controller.go_to_step(CONNECT) same as step indicator |
