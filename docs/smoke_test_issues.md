...
| 89 | Cancel button invisible in dark theme (UnsavedChangesDialog) | FIXED | NO | `47087a2` | Removed hardcoded inline styles from Cancel button; uses ghost class from QSS theme |
| 90 | Settings screen paths not pre-populated; theme change + log buttons broken | FIXED | NO | `47087a2` | _apply_settings now passes computed paths (get_log_dir, profile_repository.storage_root) when settings have empty defaults; theme wiring was already correct |
| 91 | Onboarding overlay uses hardcoded COLORS_LIGHT; text clipped in dark mode | FIXED | NO | `47087a2`, `212f672` | Overlay now rebuilds on showEvent to detect active theme; uses COLORS_DARK/COLORS_LIGHT dynamically; Skip removed |
| 92 | Pull from REW: L/R filters pushed to RoomFit profile result in empty/flat bands | FIXED | NO | `7478e7c` | Push path used naive split_lr_filters (50/50) which scrambles unequal-length channels; now stores and uses explicit filters_l/filters_r in WizardState |
| 93 | Pull from REW: subsequent pull after preset load uses stale filters from preset | OPEN | NO | `7478e7c` | Issue is still not fixed. Root cause: When using the REW API some filter values can have long decimals (can be up to 16 digits after the decimal point - for any of freq, gain or Q). The app passes those directly to WiiM devices, and they don't seem to like that in L/R mode. In Stereo mode the same thing doesn't seem to cause issues. The solution migh be to limit/normalize the number of decimal places received from REW API to something WiiM devices accept. Same root cause as #92 — the combined list was re-split incorrectly; explicit L/R lists now preserved through the push path. |
| 94 | Connect step loses checkmark/context after RoomFit push via REW API | FIXED | NO | `f5a597c` | _mark_prior_steps_completed now explicitly marks CONNECT as completed when device is selected |
| 95 | Pull from REW: Quick Setup dialog shown unnecessarily when already at Filters step | FIXED | NO | `1e4111d` | _ensure_wizard_state_for_load now checks if user is at/past FILTERS step and skips dialog; PEQ_ONLY flow no longer asks for EQ_TYPE |
| 96 | Pull from REW: measurement picker not shown (REW returns dict-keyed response) | FIXED | NO | `c496aba` | rew_http_client now handles dict-keyed measurement responses (keys "1", "2", ...) |
| 97 | Pull from REW: "Unknown filter type 'None'" error from REW API filters | FIXED | NO | `cbc8ae7` | Unified _TYPE_MAP handles all REW filter types; None/Modal/AllPass/L-T skipped; gain field reads "gaindB" |
| 98 | Push page shows stale DRY RUN content from previous dry run after real push | FIXED | NO | `a2dcafd` | PushPage.reset() called on every PUSH step entry via _on_step_changed |
...
| 100 | Hardware limit constants (gain/Q) duplicated in translator and GUI | FIXED | NO | `068caf5` | Identical _GAIN_MIN/MAX, _Q_MIN/MAX defined independently in wiim_generator.py and shared_helpers.py; consolidated into src/models/constants.py (indirectly tested via clamping tests) |
| 101 | Band-param building logic repeated 4× in wiim_adapter.py | FIXED | NO | `068caf5` | Manual loop building EQBand param dicts copy-pasted across _write_peq_batch, _write_peq_sequential, _write_peq_batch_lr, _write_peq_sequential_lr, and write_roomfit; extracted _flat_array_to_band_params() helper (indirectly tested via adapter write tests) |
| 102 | Two crash handlers overwriting each other (logging vs GUI) | FIXED | NO | `068caf5` | install_crash_handler (logging/setup.py) and _crash_handler (main_window.py) both set sys.excepthook; GUI handler now flushes all log handlers for persistence (install_crash_handler never called in production; GUI handler not unit-testable without Qt) |
...
| 112 | Help/User Guide has no visible close button; Diagnostics dock awkward UX | FIXED | NO | `b0c9900`, `2452246` | Both views now open as separate QDialog windows with native OS title bar controls; HelpView removed from stacked widget so content renders |
...
| 114 | Status banner close/dismiss button not fully visible (clipped) | FIXED | NO | `212f672`, `2452246` | Banner height 44px; cancel button has explicit border/padding; close button styled for visibility |
...
| 117 | Onboarding overlay always shows light theme; Skip button redundant | FIXED | NO | `212f672`, `2452246` | Removed Skip button; overlay rebuilds on showEvent for theme; increased label min heights |
