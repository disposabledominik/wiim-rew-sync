...
2. ~~WHEN the App launches and has previously connected to a device (last-used device stored in settings), THE App SHALL attempt to reconnect to that device first before running full discovery.~~ **Descoped 2026-06-27**: `last_device` was added to `AppSettings` and shown read-only in Settings, but auto-reconnect was never implemented and nothing ever wrote to the field. Removed entirely rather than implemented; full discovery always runs on launch.
...
15. THE Settings view SHALL include the following configurable options: log directory path, local presets library path, default REW export folder, light/dark mode preference (or "follow system"), discovery timeout (default 5 seconds), Dry Run default state for new sessions, and a "Show onboarding again" button. All settings SHALL persist across app restarts via a settings file stored in the app data directory. (Last-used device / auto-reconnect was descoped 2026-06-27 — see Requirement 9.2.)
